...
class NativeLibrary
{
    ...
    {
        if (self::$ffi !== null) {
            // FFI resources are automatically freed on script shutdown
            self::$ffi = null;
        }
    }

    /**
     * Get the path to the header file currently in use.
     */
    public static function getHeaderFile(): ?string
    {
        try {
            $path = self::getHeaderPath();
    ...
    /**
     * Get the path to the library file currently in use.
     */
    public static function getLibraryFile(): ?string
    {
        try {
            return self::findLibrary();
        } catch (\Exception $e) {
            return null;
        }
    }
