...
- **Tables: product data sheet label/value rows dropped from `extract_text` (#315)** — `extract_text` on product data sheets (ORAFOL ORALITE® 5900 and similar) dropped entire spec rows such as "Resistance to cleaning agents" and "Service life by specialist application". These PDFs draw faint cell-background rectangles behind each label/value row; the spatial table detector clustered those rectangles into tiny 2-column tables, and the caller then stripped the spans inside each table's bounding box under the assumption that the table renderer would round-trip them. It didn't: continuation lines from wrapped values produced empty-cell rows, and some continuation spans escaped the `contains_rect` filter to appear as orphan fragments ("variation" alone on a line). `is_valid_table` now rejects narrow (≤2-column) tables that contain any empty cell — a reliable signal of wrapping continuation rows in a spec sheet rather than a real 2-column data table. Genuine grid-line-detected 2×2 tables (all cells filled) continue to validate. Verified against the public ORAFOL 5900 datasheet: the Physical & Chemical Properties section now renders inline in reading order with nothing dropped.
- **Reading order: tabular content interleaved by Y jitter (#316)** — the default `TopToBottom` sort used strict Y-descending with no tolerance, so any sub-point Y variation between cells in the same row — font-metric jitter, superscripts, centered multi-row labels, CJK glyph centering — interleaved them with the next row when building output. A Chinese blood-cell reference-range table showed columns entirely dissociated from their row labels. Added `row_aware_span_cmp` in `utils`: it quantises top-Y into 3pt bands (matching typographic baseline jitter for 10–12pt body text without merging adjacent 14pt-leading lines), sorts band-descending then X-ascending, and uses `i32` band keys so the ordering is a valid total order for `sort_by`. Wired into every `TopToBottom` sort site across `document.rs`. As a targeted follow-up for centered labels that vertically span multiple data rows, `PdfDocument::reorder_rowspan_labels` also promotes sparse-column spans that sit inside the data Y range of the dense columns to the top of their label block, so rowspan labels emit in row order with their data instead of floating inside the band of the first data row.
...
- **@Goldziher** — Reported four extraction issues (#313, #314, #315, #316) with clear repro snippets that let us localise the AES-256 string-decryption gap, the Algorithm 2.B termination off-by-one, the single-column XYCut fragmentation, the narrow-table false positive on spec sheets, and the row-aware sort gap for tabular content.
