...
- **A paragraph set in per-word runs was cut down an aligned word gap, and a hyphenated word lost its second half** — the crossed-corridor guard (below) lets a column cut stand when a run crosses the corridor but both sides are near-full-height columns, because a banner over two columns crosses every corridor between them. On a magazine page whose justified paragraph is emitted as one run per word — the word gaps exceed the merge threshold — one gap at x≈200 aligned down three lines above a letter-spaced listing; the two full lines around it crossed the corridor, the halves measured 0.79 of the region, which the widened column-height bar admits, and the cut was taken: `pro-` was emitted with the listing's right-hand fragments, three lines from `ceeds`, and `proceeds` left the page. A banner sits above the columns it crosses; a run whose centre line lies *between* the rows of both sides, by at least half its own height, is a line of the same text as the rows around it, and the cut is now refused there. A run at the top or bottom edge of either side is still furniture, so a heading over columns that start on the next line keeps its cut; a full-measure element genuinely between column rows carries the white space a typesetter gives it, which is a row valley the recursion peels first. §9.4.4 has a horizontal run occupy one unbroken interval on the writing axis, so a full line between two rows of a "column" is proof there is no corridor there. Sourced by bisecting the release range to the column-height change; v0.3.77's own chopping of the page's letter-spaced listing is unchanged (#1340).
