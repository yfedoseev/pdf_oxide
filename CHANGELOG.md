...
- **Decimal points and punctuation in symbolic math fonts no longer decode as stray symbols (#536)** — a TeX math font (e.g. CMMI10) that parks the `period` / `comma` / `hyphen` / `minus` glyph at a non-standard code via `/Differences` (ISO 32000-1:2008 §9.6.6.1) previously decoded a figure axis like `0.75` as `0¬75` / `0:75`. When the resolved Unicode is a non-sensible symbol **and** the code's authoritative glyph name is one of those four punctuation names, the Adobe Glyph List value (§9.10.2) is now preferred. Tightly gated: correctly-mapped fonts and genuine `logicalnot` (`¬`) glyphs are untouched.
