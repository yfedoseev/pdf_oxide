...
static NSData *samplePdf(void) {
    NSError *err = nil;
    POXPdf *p = [POXPdf fromMarkdown:@"# Coverage Doc\n\nAlpha bravo charlie. Some **bold** text.\n" error:&err];
...
int main(void) {
    @autoreleasepool {
        ...
        CHECK([[[POXPdf fromHtml:@"<h1>h</h1><p>b</p>" error:&err] saveToBytesError:&err] length] > 100);
        CHECK([[[POXPdf fromText:@"plain text body" error:&err] saveToBytesError:&err] length] > 100);
        {
            NSString *path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"pdfoxide_objc.pdf"];
        ...
        {
            NSString *path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"pdfoxide_objc_open.pdf"];
        ...
        CHECK([[doc extractStructuredJson:0 error:&err] length] > 0);  // extractStructuredJson
