...
fn main() {
    ...
    let format = std::env::args().nth(2).unwrap_or_else(|| "text".to_string());
    ...
    let doc = match pdf_oxide::document::PdfDocument::from_bytes(bytes) {
        Ok(d) => d,
        Err(e) => { eprintln!("error parsing {path}: {e}"); std::process::exit(1); }
    };
    let _ = doc.authenticate(b"");

    let opts = ConversionOptions { extract_tables: true, ..Default::default() };
    ...
    match format.as_str() {
        ...
        "markdown" => {
            match doc.to_markdown_all(&opts) {
                Ok(md) => print!("{md}"),
                Err(e) => { eprintln!("markdown error: {e}"); std::process::exit(1); }
            }
        }
        "html" => {
            match doc.to_html_all(&opts) {
                Ok(html) => print!("{html}"),
                Err(e) => { eprintln!("html error: {e}"); std::process::exit(1); }
            }
        }
        other => {
