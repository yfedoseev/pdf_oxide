...
fn main() {
    let tests: &[(&str, &str, &str, f32)] = &[
        ("hello_structure",   "/tmp/hello_structure.pdf",   "/tmp/gt_hello_structure.txt",    0.88),
        ...
        ("issue-336-example", "/tmp/issue-336-example.pdf", "/tmp/gt_issue-336-example.txt",  0.69),
    ...
    for (name, pdf_path, gt_path, threshold) in tests {
        let bytes = match std::fs::read(pdf_path) {
            Ok(b) => b,
            Err(_) => { println!("SKIP {name}: pdf not found"); continue; }
        };
        let gt = match std::fs::read_to_string(gt_path) {
            Ok(t) => t,
            Err(_) => { println!("SKIP {name}: gt not found"); continue; }
        };
        let doc = match pdf_oxide::document::PdfDocument::from_bytes(bytes) {
            Ok(d) => d,
            Err(e) => { println!("ERR  {name}: {e}"); continue; }
        };
