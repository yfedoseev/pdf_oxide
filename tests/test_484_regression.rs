//! Regression tests for GitHub issue #484.
//!
//! Covers the specific bugs that were reported and fixed:
//!
//! * Section 1 — extraction crashes on annotation-heavy and rotated PDFs:
//!   - Bare-word identifiers (e.g. `OBJR` without a leading `/`) crashed the
//!     parser; they are now treated as Name tokens (lenient fallback).
//!   - A corrupt or unresolvable StructTreeRoot now returns `Ok(None)` instead
//!     of `Err(InvalidPdf)`, allowing graceful fallback to font-size clustering.
//!
//! * Section 2 — near-empty/empty extraction on valid PDFs:
//!   - Encrypted PDFs returned `Ok(0)` from `page_count()` instead of
//!     `Err(EncryptedPdf)`, causing callers to silently skip all pages.
//!   - CJK CID-keyed fonts without ToUnicode produced Korean Hangul instead
//!     of the correct Chinese characters.
