...
#[test]
fn a_relative_jump_ends_the_run() {
    let s = spans(JUMP_TD);
...
#[test]
fn an_absolute_jump_ends_the_run_too() {
    let s = spans(JUMP_TM);
    assert_eq!(
        s.len(),
        2,
...
#[test]
fn the_two_forms_of_the_same_jump_agree() {
...
#[test]
fn a_span_does_not_claim_the_gap_as_its_width() {
    let s = spans(JUMP_TM);
    let first = &s[0];
    assert!(
        first.2 < 40.0,
...
#[test]
fn a_glyph_by_glyph_run_still_batches_into_one_span() {
...
#[test]
fn a_word_sized_gap_is_not_a_jump() {
...
#[test]
fn a_seam_wider_than_the_space_advance_still_belongs_to_the_merger() {
