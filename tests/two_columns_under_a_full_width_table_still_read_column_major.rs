...
#[test]
fn the_fixture_is_two_columns_over_a_table_that_blankets_the_channel() {
...
#[test]
fn the_body_reads_column_major_under_the_table() {
    let doc = PdfDocument::from_bytes(two_column_page_over_a_table()).expect("open");
...
#[test]
fn the_wrapped_word_in_the_right_column_survives() {
    let doc = PdfDocument::from_bytes(two_column_page_over_a_table()).expect("open");
    let out = doc.extract_text(0).expect("text");
    assert!(
