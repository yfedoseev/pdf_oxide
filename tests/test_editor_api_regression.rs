...
fn overlay_add_text_preserves_original_text_and_graphics() {
    ...
    let overlay_present = all.contains("overlay text")
        || bytes.windows(12).any(|w| w == b"overlay text");
...
fn overlay_add_text_after_select_pages() {
    ...
    let overlay_present = all.contains("post-select overlay")
        || bytes.windows(20).any(|w| w == b"post-select overlay\n".get(..w.len()).unwrap_or(&[]));
...
fn overlay_add_text_after_select_pages_last_page() {
    ...
    let has_overlay = all.contains("third-page-overlay")
        || bytes.windows(18).any(|w| w == b"third-page-overlay");
...
fn get_page_rotation_consistent_after_select_pages() {
    ...
    assert_eq!(rotation, 270, "get_page_rotation did not reflect set_page_rotation after select_pages");
}
...
fn set_page_media_box_after_select_pages() {
    ...
    editor.select_pages(&[1]).expect("select_pages");
    editor.set_page_media_box(0, [0.0, 0.0, 400.0, 600.0]).expect("set_page_media_box");

    let mb = editor.get_page_media_box(0).expect("get_page_media_box");
    assert_eq!(mb, [0.0, 0.0, 400.0, 600.0], "media_box not reflected by getter after select_pages");
...
fn set_page_crop_box_after_select_pages() {
    ...
    editor.set_page_crop_box(0, [10.0, 10.0, 590.0, 780.0]).expect("set_page_crop_box");
...
fn erase_then_add_text_on_existing_page() {
    ...
    editor.erase_region(0, [100.0, 600.0, 200.0, 100.0]).expect("erase_region");
